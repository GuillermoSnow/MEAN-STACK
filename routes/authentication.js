...
module.exports = (router) => {
  ...
  router.post('/login', (req, res) => {
    // Check if username was provided
    if (!req.body.username) {
      res.json({ success: false, message: 'No se ingresó usuario' }); // Return error
    } else {
      // Check if password was provided
      if (!req.body.password) {
        res.json({ success: false, message: 'No se ingresó contraseña.' }); // Return error
      } else {
        ...
        User.findOne({ username: req.body.username.toLowerCase() }, (err, user) => {
          ...
          if (err) {
            ...
            if (!user) {
              res.json({ success: false, message: 'No se encontró el usuario.' }); // Return error
            } else {
              const validPassword = user.comparePassword(req.body.password); // Compare password provided to password in database
              // Check if password is a match
              if (!validPassword) {
                res.json({ success: false, message: 'Contraseña incorrecta' }); // Return error
              } else {
